...
public class CornerUtil {
    ...
    public static void setArenaCorners(String arenaName, Player player, int corner) {
        ...
        if (arena != null) {
            ...
            Location cornerLocation = player.getLocation();

            Block blockUnderPlayer = cornerLocation.clone().subtract(0, 1, 0).getBlock();

            if (blockUnderPlayer.getType().equals(Material.AIR)) {
                player.sendMessage(StringUtil.CC("&cBlock not found!"));
                return;
            }
