...
public class SetHunger
{
    ...
    public static void run(Player player, String label, String[] args)
    {
        if (args.length != 2)
        {
            player.sendMessage(StringUtil.CC("&c/" + label + " sethunger <ladder_id>/<ladder_name>"));
            return;
        }
        ...
        if (ladder.isHunger())
        {
            ladder.setHunger(false);
            player.sendMessage(StringUtil.CC("&eYou &cdisabled &ethe ID" + ladder.getId() + " &eladder's hunger setting."));
        }
        else
        {
            ladder.setHunger(true);
            player.sendMessage(StringUtil.CC("&eYou &eenabled &ethe ID" + ladder.getId() + " &eladder's hunger setting."));
        }
    }
