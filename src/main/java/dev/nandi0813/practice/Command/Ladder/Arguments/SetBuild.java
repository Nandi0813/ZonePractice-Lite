...
public class SetBuild
{
    ...
    public static void run(Player player, String label, String[] args)
    {
        if (args.length != 2)
        {
            player.sendMessage(StringUtil.CC("&c/" + label + " setbuild <ladder_id>/<ladder_name>"));
            return;
        }
        ...
        if (ladder.isBuild())
        {
            ladder.setBuild(false);
            player.sendMessage(StringUtil.CC("&eYou &cdisabled &ethe ID" + ladder.getId() + " &eladder's build setting."));
        }
        else
        {
            ladder.setBuild(true);
            player.sendMessage(StringUtil.CC("&eYou &eenabled &ethe ID" + ladder.getId() + " &eladder's build setting."));
        }
    }
