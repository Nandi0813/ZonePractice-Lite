...
public class SetRanked
{
    ...
    public static void run(Player player, String label, String[] args)
    {
        if (args.length != 2)
        {
            player.sendMessage(StringUtil.CC("&c/" + label + " setranked <ladder_id>/<ladder_name>"));
            return;
        }
        ...
        if (ladder.isRanked())
        {
            ...
            player.sendMessage(StringUtil.CC("&eYou &cdisabled &ethe ID" + ladder.getId() + " &eladder's ranked setting."));
        }
        else
        {
            ladder.setRanked(true);
            player.sendMessage(StringUtil.CC("&eYou &eenabled &ethe ID" + ladder.getId() + " &eladder's ranked setting."));
        }
    }
